...
```
* 2、创建requirements.txt，其中包含了python依赖库，可以通过`pip freeze>requirements.txt`生成，不过这样会将当前python环境所有的依赖都会写入requirements.txt文件中；因此可以安装pipreqs库，自动分析该项目需要的库文件
* 3、创建镜像`docker image build -t django-docker .`其中django-docker为镜像名称
